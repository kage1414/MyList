...
func (i *ItemQuery) collectField(ctx context.Context, opCtx *graphql.OperationContext, collected graphql.CollectedField, path []string, satisfies ...string) error {
	...
	for _, field := range graphql.CollectFields(opCtx, collected.Selections, satisfies) {
		...
		case "priority":
			if _, ok := fieldSeen[item.FieldPriority]; !ok {
				selectedFields = append(selectedFields, item.FieldPriority)
				fieldSeen[item.FieldPriority] = struct{}{}
			}
		case "complete":
			if _, ok := fieldSeen[item.FieldComplete]; !ok {
				selectedFields = append(selectedFields, item.FieldComplete)
				fieldSeen[item.FieldComplete] = struct{}{}
			}
		case "id":
		case "__typename":
		default:
